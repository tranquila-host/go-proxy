...
**Публичный тест-сервер (на ваш страх и риск):** `144.31.221.149`  
(Мы постараемся поддерживать его включённым; использование публичного сервера — риск для безопасности и приватности ваших данных.)
...
- Оператор любого ретранслятора видит **весь** проходящий трафик: URL, параметры, заголовки, тело запросов, куки и API-ключи.
...
- Рекомендации:
  - По возможности используйте **собственный сервер**, которому вы доверяете.
  - Не отправляйте через публичные ретрансляторы секреты/ключи.
  - Ограничьте доступ к вашему прокси (firewall, basic auth, IP-whitelist).

---

## Почему НЕ стоит использовать публичный ретранслятор

- Потенциальная кража API-ключей.
