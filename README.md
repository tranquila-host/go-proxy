...
- Рекомендации:
  ...
  - Рассмотрите шифрование чувствительной части данных на уровне приложения (если возможно).
...
- Подмена ответов — возможный вектор для мошенничества.
- Логирование и утечка метрик/истории запросов.
...
- Если вы хотите, чтобы прокси не логировал тело запросов — убедитесь, что Go-программа не пишет body в логи и что на сервере нет pcap/tcpdump, сохраняющего трафик.
- Если origin использует Cloudflare и SNI/ECH, прокси должен делать запросы по имени хоста (target) — это предусмотрено `-target`.
- Можно добавить простую авторизацию (Basic / Bearer) на nginx, чтобы ограничить доступ к прокси.
- Рассмотрите rate-limit.
